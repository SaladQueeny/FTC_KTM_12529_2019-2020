 ...
 *YES
 */
@Autonomous(name= "Auto_redskystoneandfoundation", group="AutoOP")
//@Disabled//comment out this line before using
public class Auto_redskystoneandfoundation extends robot {
...
//            moveDistance(0.4, 700);
        ...
        waitForStart();{
            ...
            if(pologenieskystone=="left")
            {
                ...
                time1=getRuntime();
                time2=getRuntime();
                while(!isStopRequested()&&DistanceSensor_back.getDistance(DistanceUnit.CM)<59){
                    setMotorsPower(-0.35*koeff,0.35*koeff,0.35*koeff,-0.35*koeff);//
                    //sleep(100);
                }
                setMotorsPower(0,0,0,0);
                /*while(!isStopRequested()&&time2-time1<1.11){
                ...
                time1=getRuntime();
                time2=getRuntime();
                while(!isStopRequested()&&time2-time1<0.82){
            ...
            if(pologenieskystone=="right")
            {
                time1=getRuntime();
                time2=getRuntime();
                while(!isStopRequested()&&DistanceSensor_back.getDistance(DistanceUnit.CM)<57){
                    setMotorsPower(-0.35*koeff,0.35*koeff,0.35*koeff,-0.35*koeff);//
                    //sleep(100);
                }
                setMotorsPower(0,0,0,0);
                ...
                time1=getRuntime();
                time2=getRuntime();
                while(!isStopRequested()&&time2-time1<1.27){
            ...
            time1=getRuntime();
            time2=getRuntime();
            while(!isStopRequested()&&time2-time1<0.6*pologenieskystone1+1.2+rl*0.13-rv*0.15){
                setMotorsPowerforvard(-0.6*koeff,0.6*koeff,0.6*koeff,-0.6*koeff,angles, imu, -91);
                time2=getRuntime();
            }
            ...
            //sleep(100);
            setMotorsPowerTimed(-0.3 * koeff, 0.3 * koeff, 0.3 * koeff, -0.3 * koeff, 550);//
            //sleep(100);
            ...
            time1=getRuntime();
            time2=getRuntime();
            while(!isStopRequested()&&time2-time1<0.9){
            ...
            //sleep(100);
            while(!isStopRequested()&&angles.firstAngle>-82){
                setMotorsPower(0.55*koeff,0.55*koeff,0.55*koeff,0.55*koeff);
                angles=imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
            }
            setMotorsPower(0,0,0,0);
            //setMotorsPowerTimed(-0.3 * koeff, -0.3 * koeff, -0.3 * koeff, -0.3 * koeff, 940);//povorot
            //sleep(100);
            setMotorsPowerTimed(-0.4 * koeff, 0.4 * koeff, 0.4 * koeff, -0.4 * koeff, 500);//
            sleep(100);
            s5Shovel.setPosition(0);
            sleep(100);
            //setMotorsPower(0,0,0,0);
            ...
            /*while(!isStopRequested()&&DistanceSensor_right.getDistance(DistanceUnit.CM)>85){
...
*/
            ...
            time1=getRuntime();
            time2=getRuntime();
            while(!isStopRequested()&&time2-time1<0.6*pologenieskystone1+0.95){
                setMotorsPowerback(0.8*koeff,-0.8*koeff,-0.8*koeff,0.8*koeff,angles, imu, -84);
                time2=getRuntime();
            }
            //setMotorsPower(0,0,0,0);
            //sleep(200);
            ...
            if(pologenieskystone=="right")
            {
                ...
                setMotorsPower(0,0,0,0);
                sleep(100);*/
                while(!isStopRequested()&&DistanceSensor_left.getDistance(DistanceUnit.CM)>50){
                    setMotorsPowerleft(0.4*koeff,-0.4*koeff,0.4*koeff,-0.4*koeff,angles, imu, 0);
                }
                setMotorsPower(0,0,0,0);
                sleep(50);
                while(!isStopRequested()&&DistanceSensor_back.getDistance(DistanceUnit.CM)<67){
            ...
            if(pologenieskystone=="middle")
            {
                ...
                setMotorsPower(0,0,0,0);
                sleep(100);*/
                while(!isStopRequested()&&DistanceSensor_left.getDistance(DistanceUnit.CM)>25){
                    setMotorsPowerleft(0.4*koeff,-0.4*koeff,0.4*koeff,-0.4*koeff,angles, imu, 0);
                }
                setMotorsPower(0,0,0,0);
                sleep(50);
            ...
            if(pologenieskystone=="left")
            {
                ...
                sleep(100);*/
                right =1;
                while(!isStopRequested()&&DistanceSensor_left.getDistance(DistanceUnit.CM)>15){
                    setMotorsPowerleft(0.4*koeff,-0.4*koeff,0.4*koeff,-0.4*koeff,angles, imu, 0);
                }
                setMotorsPower(0,0,0,0);
                sleep(50);
                ...
                sleep(500);
                while(!isStopRequested()&&angles.firstAngle<24){
                    setMotorsPower(-0.35*koeff,-0.35*koeff,-0.35*koeff,-0.35*koeff);
                    angles=imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
                }
                setMotorsPower(0,0,0,0);
            ...
            time1=getRuntime();
            time2=getRuntime();
            while(!isStopRequested()&&time2-time1<0.6*pologenieskystone1+1.6-right*0.2){
                setMotorsPowerforvard(-0.6*koeff,0.6*koeff,0.6*koeff,-0.6*koeff,angles, imu, -93);
                time2=getRuntime();
            }
            //setMotorsPower(0,0,0,0);
            //sleep(200);
            ...
            //sleep(100);
            otpustiskystone();
            while(!isStopRequested()&&DistanceSensor_right.getDistance(DistanceUnit.CM)<67){
                setMotorsPowerleft(0.4*koeff,-0.4*koeff,0.4*koeff,-0.4*koeff,angles,imu,-90);//
                //sleep(100);
            }
            setMotorsPower(0,0,0,0);
            ...
            time2=getRuntime();
            while(!isStopRequested()&&time2-time1<0.9){
                setMotorsPowerforvard(0.8*koeff,-0.8*koeff,-0.8*koeff,0.8*koeff,angles, imu, -84);
                time2=getRuntime();
            }
            setMotorsPower(0,0,0,0);
